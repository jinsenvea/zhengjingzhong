...
var _this=this;
              ...
              verifyOpenid(commonParams.searxhshopsParams({
                   code:arrCode[0]
          ...
          //跳转到补齐资料页面  F:\mint20\src\components\rziliao
                    setTimeout(function(){
                      _this.$router.push({path:'/rziliao'})
                    },1200)
      ...
      changeLenth(){
         var _this=this;
           ...
           if(this.phone.length>=11){
             //改变状态
          // 验证用户是否可以注册
          verifyPhoneNum(commonParams.searxhshopsParams({
           phone:this.phone
          })).then((res)=>{
            console.log(res)
            if(res.data.status==2){
                this.$store.dispatch('toast',"恭喜你，可以注册");
                ...
                this.$store.dispatch('toast',res.data.message);
              //  this.verifyPhoneNumState=false
            }
          })
                // 
           ...
           console.log(res)
            ...
            if(res.data.status==1){
              //跳转到修改密码页面
					  ...
					  this.$router.push({path:'/registerDetail',query:{title:'详细信息',phone:this.phone,password:hexMD5(this.passwork)}})

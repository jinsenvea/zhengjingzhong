...
import {getPolt,addUserInfo} from 'api/editAdress'
...
//此处可以优化
 ...
 this.popupAreanow=false
        ...
        if(this.regAitive==''){
       _this.$store.dispatch('toast',"请选择年龄段");
        }else{
                addUserInfo(commonParams.searxhshopsParams({
          ...
          if(res.data.status==1){
             ...
             if(this.$store.state.pay.redirectPath!=''&&this.$store.state.pay.redirectPath!=null){
                _this.$store.dispatch('toast',"恭喜你注册成功");
